	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.cors().and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().csrf()
				...
				.antMatchers("/v2/api-docs", "/configuration/ui", "/swagger-resources/**", "/configuration/security",
						...
						"/api/auth/login","/api/basicMaster/leavetype","/api/basicMaster/leave/balance","/api/basicMaster/leave/balance/{empcode}","/api/basicMaster/leavecredit","/api/basicMaster/leavetype/{id}","/api/basicMaster/employee/daily/status","/api/basicMaster/attendance/{empcode}")
